...
Bases de Datos.
Los objetos de las bases de datos y ellas mismas deben ser nombrados con prefijos que representen el tipo de objeto que se está creando, para la nombrar las bases de datos se seguirán las reglas:
o	Utilizar sustantivos.
o	Usar all upper case. 
o	Hacer uso del prefijo “BD”.
o	Usar palabras que identifiquen el objetivo de la base de datos.
Ejemplo:
...
Ejemplo:
“prefijo” + “nombre de tabla”
cCatalogo 

	Nombre de Tabla	Prefijo
1	Control de Catálogos Estáticos	ce
2	Control de Catálogos Parametrizables	ct
3	Control de Tablas de Sistema	ts
4	Control de Tablas de Log	tl - tlog 
5	Control de Tablas Compuestas	tr
6	Control de Históricos	th


Se debe definir un estándar por proyecto para controlar el agrupamiento de las tablas, estas pueden ser definidas por modulo, por ejemplo:
SOP
	Nombre de Tabla	Prefijo
1	Control de Tablas exclusivas para Aéreo	taer
2	Control de Tablas exclusivas para Marítimo	tmar
3	Control de Tablas exclusivas para Terrestre	tter
...
En Transact-SQL existen varios tipos de datos, mismos que han sido clasificados de la siguiente manera para asignarles un prefijo, que será utilizado para nombrar las variables según el tipo de dato bajo el que se declaren.
	TIPO	PREFIJO	DESCRIPCION	TIPO DE DATOS
1	NUMERICO ENTERO	i	Tipo de datos utilizados como entero	Bit
				Tinyint
				Smallint
				Int
				BigInt
2	NUMERICO DECIMAL	d	Tipo de datos utilizados con decimales	Decimal
				Float
				Real
3	NUMERICO MONETARIO	m	Tipo de datos utilizados como numéricos monetarios	Money
				SmallMoney
4	FECHA HORA	dt	Tipo de datos utilizados para la fecha y hora	Date
				Datetime
				SmallDatetime
				TimeStamp
5	CADENA	s	Tipo de datos utilizados para cadena de caracteres	Char
				Varchar
				Nchar
				Nvarchar
6	BINARIOS	b	Tipo de datos utilizados como binarios	Binary
				Varbinary
...
También se manejaran algunas opciones en SQL los cuales son Funciones, Procedimientos Almacenados, Vistas y Disparadores. Para cada uno de estos se asignará un prefijo:
	TIPO	PREFIJO
1	Disparador – Trigger	tr
2	Procedimiento Almacenado - Store Procedure	pa
3	Vista - View	vs
4	Función - Function	fu
...
vsEmpresa
Recomendación.
Se recomienda usar el prefijo de modulo como forma para identificar fácilmente o seccionar y clasificar los objetos en la base.
“prefijo” + “PrefijoModulo” + “NombreObjeto”
Ejemplo – prefijo modulo: s – sistema
vsSisEmpresa  **Indicar el modulo al final del nombre a consideración
 
Documentar Objetos.
Todos los objetos deberán ser documentados utilizando un comentario en la parte superior de la definición los objetos con el fin de describir el objetivo de cada uno y poner en contexto al programador sobre du funcionalidad. Ejemplo: 
/* El objetivo principal de este [nombre objeto] es el de [breve descripción]*/ Seguido por la declaración del objeto. 
...
Al agregarse comentarios en el código, deberán situarse justo en la línea superior de la instrucción o sentencia, sin líneas en blanco de por medio. Ejemplo: 
...
o	Renombrar todos los campos según los estándares definidos de columnas, para que al consultarse tengan un nombre asignado, en caso de que la consulta conserve el nombre original este paso puede omitirse.
o	Uso de diccionario de datos, según el formato definido.
o	Evitar el uso del comodín (*) en bloques SELECT o INSERT. Siempre hacer referencia a las columnas involucradas. Esto evitara los errores, si es que la tabla es modificada agregando columnas.
o	Agrupar la vista de forma que sea posible visualizarla en pantalla sin necesidad del scroll horizontal, es decir, mantener una longitud coherente para las líneas de código.
o	No se deberá hacer uso de cursores.
o	Las columnas usadas en la comparación del bloque JOIN deberán ser indexados.
o	Evitar el uso de índices duplicados.
o	Se recomienda el uso de indexado en columnas con alta recurrencia en consultas.
o	Minimizar el uso de tablas temporales, en su caso optar por el uso de variables de tipo tabla.
o	Realizar un salto de línea para identificar cada bloque dentro de las sentencias SQL.
Ejemplo:
...
/* Es recomendable hacer uso del tipo de dato correcto para cada columna disminuyendo la manipulación de datos */
